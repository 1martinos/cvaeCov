...
if __name__ == '__main__':
    ...
    eval_data = torch.tensor(data[choices_eval]).float().to(device)
    ...
    test_data = torch.tensor(data[choices_test]).float().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():        # Test for each Epoch
            reconstruct_x, mean,log_var, z = cvae(eval_data[:100])
            file_dir = f"./models/cov{pool_size}-{cur_date}"
            if not os.path.isdir(file_dir):
                os.makedirs(file_dir)
            with open(f"./{file_dir}/{epoch}.p","wb") as f:
                pickle.dump((reconstruct_x.cpu(),eval_data.cpu()),f)
        print(torch.cuda.get_device_name(0))
        print('Memory Usage:')
        print('Allocated:', round(torch.cuda.memory_allocated(0) / 1024 ** 3, 1), 'GB')
        print('Cached:   ', round(torch.cuda.memory_reserved(0) / 1024 ** 3, 1), 'GB')
